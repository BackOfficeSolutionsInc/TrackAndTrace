...
namespace TractionTools.Tests.API.v1 {
	[TestClass]
	public class RolesApiTests_v1 : BaseApiTest {
		public RolesApiTests_v1() : base(VERSION_1) { }
		...
		public async Task TestUpdateRoles() {
			...
			//Updated
			await roleController.UpdateRoles(addRole.Id, new TitleModel { title = name });
		...
		public async Task TestRemoveRoles() {
			...
			//remove role
			await roleController.RemoveRoles(addRole.Id);
