...
namespace TractionTools.Tests.Utilities {
    ...
    public class SyncTests : BaseTest {
        ...
        [TestMethod]
        public async Task TestEnsureStrictlyAfter() {
            using (HibernateSession.SetDatabaseEnv_TestOnly(Env.local_mysql)) {
                var org = await OrgUtil.CreateOrganization();

                var key = SyncAction.TEST("TestEnsureStrictlyAfter");
                ...
                var firstHook = new SyncUtil.TestHooks() {
                    BeforePreCheck = () => {
                        Assert.IsFalse(secondStarted);
                        Assert.IsFalse(secondFinished);
                    },
                    AfterPreCheck = () => {
                        Assert.IsFalse(secondStarted);
                        Assert.IsFalse(secondFinished);
                ...
                var firstUpdate = Task.Run(async () => {
                    Assert.IsFalse(secondStarted);
                    await SyncUtil.EnsureStrictlyAfter(org.Manager.SetClientTimeStamp(1), s => key, async x => {
                        firstStarted = true;
                ...
                var secondHook = new SyncUtil.TestHooks() {
                    BeforePreCheck = () => {
                        Assert.IsTrue(firstStarted);
                        Assert.IsFalse(firstFinished);
                    },
                    AfterPreCheck = () => {
                        Assert.IsTrue(firstStarted);
                        Assert.IsFalse(firstFinished);
                ...
                var secondUpdate = Task.Run(async () => {
                    ...
                    await SyncUtil.EnsureStrictlyAfter(org.Manager.SetClientTimeStamp(2), s => key, async x => {
                        secondStarted = true;
                        await Task.Delay(1);
                    }, hooks: secondHook);
                    secondFinished = true;
                    Assert.IsTrue(firstStarted);
                    Assert.IsTrue(firstFinished);
                });
                ...
                Assert.IsTrue(secondStarted);
                Assert.IsTrue(firstFinished);
                Assert.IsTrue(secondFinished);
            }
        }

        [TestMethod]
        public async Task TryToGetValueWhileItsLocked() {
            using (HibernateSession.SetDatabaseEnv_TestOnly(Env.local_mysql)) {
                ...
                var aquireFinished = false;
                var tryGetFinished = false;

                var order = new int[4];
                var i = 0;

                var aquireLock = Task.Run(() => {
                    Console.WriteLine("a...");
                    using (var s = HibernateSession.GetCurrentSession()) {
                        Console.WriteLine("a...2");
                        using (var tx = s.BeginTransaction()) {
                            Console.WriteLine("0 - AquireLock-Start");
                            order[0] = (i++);
                            var a = s.Get<SyncLock>(syncLock.Id, LockMode.Upgrade);
                            a.UpdateCount += 1;
                            Task.Delay(5000);
                            aquireFinished = true;
                            Console.WriteLine("3 -AquireLock-Done");
                            Assert.IsTrue(tryGetFinished);
                            order[3] = (i++);
                ...
                var tryGet = Task.Run(() => {
                    ...
                    using (var s = HibernateSession.GetCurrentSession()) {
                        ...
                        using (var tx = s.BeginTransaction()) {
                            Task.Delay(2500);
                            order[1] = (i++);
                            Console.WriteLine("1 - TryGet-Start");
                            s.Get<SyncLock>(syncLock.Id);
                            tryGetFinished = true;
                            Console.WriteLine("2 -TryGet-Done");
                            Assert.IsFalse(aquireFinished);
                            order[2] = (i++);
                ...
                await Task.WhenAll(new[] { aquireLock, tryGet });

                Assert.IsTrue(tryGetFinished);
                Assert.IsTrue(aquireFinished);

                for (var j = 0; j < order.Length; j++) {
                    Assert.AreEqual(j, order[j]);
                }
            }
        }
