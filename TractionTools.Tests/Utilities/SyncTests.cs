    ...
    public class SyncTests : BaseTest {
            ...
            using (HibernateSession.SetDatabaseEnv_TestOnly(Env.local_mysql)) {
                ...
                for (var i = 0; i < 100; i++) {
                    ...
                    list.Add(Task.Run(() => {
                        ...
                        return SyncUtil.EnsureStrictlyAfter(org.Manager, SyncAction.UpdateScore(id,DateTime.MaxValue,1), async s => { await Task.Delay(0); },
        ...
        public async Task TestMutlipleSimultaniousAccessSync_DifferentId() {
            using (HibernateSession.SetDatabaseEnv_TestOnly(Env.local_mysql)) {
                ...
                for (var i = 0; i < 100; i++) {
                    var j = i;
                    list.Add(Task.Run(()=>SyncUtil.EnsureStrictlyAfter(org.Manager, SyncAction.UpdateScore(1 + j, DateTime.MaxValue, 1), async s => { await Task.Delay(0); })));
                }
