...
namespace TractionTools.Tests.Crosscutting.Events {
	...
	public class EventProcessorTests {
		...
		public void TestDaysWithoutL10() {
			...
			var settings = new BaseEventSettings(null,0,new DateTime(2017,1,15));
			...
			settings.SetLookup(new SearchRealL10Meeting(0).UniqueKey(settings), meetings);
			settings.SetLookup(new SearchL10RecurrenceIds().UniqueKey(settings), new List<long> { 0 });


			Assert.IsTrue(EventProcessor.ShouldTrigger(settings, evtGen));

			settings = new BaseEventSettings(null, 0, new DateTime(2017, 2, 15));
			settings.SetLookup(new SearchRealL10Meeting(0).UniqueKey(settings), meetings);
			settings.SetLookup(new SearchL10RecurrenceIds().UniqueKey(settings), new List<long> { 0 });
