            ...
            using (var s = HibernateSession.GetCurrentSession()) {
                using (var tx = s.BeginTransaction()) {
                    ...
                    for (var remainder = 0; remainder < divisor; remainder += 1) {
                        var todos = SchedulerController.TodoEmailHelpers._QueryTodoModulo(s, divisor, remainder, rangeLow, rangeHigh, nextWeek);
                    ...
                    for (var remainder = 0; remainder < divisor; remainder += 1) {
                        ...
                        await SchedulerController.TodoEmailHelpers._ConstructTodoEmails(emailTime+1, unsentMail, s, nowUtc, divisor, remainder);
                        Assert.AreEqual(0, unsentMail.Count);
                    }

                    for (var remainder = 0; remainder < divisor; remainder += 1) {
                        var unsentMail = new List<Mail>();
                        await SchedulerController.TodoEmailHelpers._ConstructTodoEmails(emailTime, unsentMail, s, nowUtc, divisor, remainder);
