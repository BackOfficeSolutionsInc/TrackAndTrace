...
function inject_setup() {
	...
	function startup() {
		...
		onMatch(HAPPYFOX_TICKET_LIST_URL, function () {
			console.log("here");
	...
	function categoryBuilder(parent,categories,onclick,showAdd){
		(function(){
			$(parent).html("");
			var tlc = $("<div>").addClass("inject-tlc-cats");
			var childBox = $("<div>").addClass("inject-child-cats");
			$(parent).append(tlc);
			$(parent).append(childBox);
			for(var i=0;i<categories.length;i++){
				(function(){
				var c = categories[i];
				var dom = $("<div title='"+c.Name+"' data-id='"+c.Id+"'>"+c.Name+"</div>").addClass("inject-cat");
				var p = parent;
				var children =c._Children;
				if (children.length>0){
					dom.addClass("inject-has-child-cat");
				}else{					
					dom.addClass("inject-leaf-cat");
				}
				
				dom.on("click",function(){
					if (c._Children.length>0){
						categoryBuilder(childBox,c._Children,onclick,showAdd);
					}
					onclick(c,$(this));					
				});
				$(tlc).append(dom);
			...
			/*if(showAdd==true){
				var dom = $("<div title='Add category' >Add...</div>").addClass("inject-cat inject-add-cat");
				dom.on("click",function(){
					addCategory(parent
				});
			}*/
	...
	async function executeCategory(cat){
		console.log("exe",cat);
		await executeCategoryEmail(cat);
		$(".cke_wysiwyg_div").addClass("inject-waiting");
		await executeCategoryTags(cat);
		$(".cke_wysiwyg_div").removeClass("inject-waiting");
	}
	...
	async function tryAddGreeting(){
		var wysiwyg =$(".floating-editor_editor-pane .cke_wysiwyg_div");
		try{
	 ...
	 function addReplyAlls() {
		$(".updates-section-body article[data-test-id='ticket-update']:not(.inject-reply-all-row)").each(function () {
			
			$(this).addClass("inject-reply-all-row");
			if ($(this).find(".mod-activity-messages").length>0){
				return;
			}

			var replyAll = $("<div>reply all</div>");
			replyAll.addClass("inject-reply-all");
			$(this).find("header").append(replyAll);
			$(replyAll).on("click", function (e) {
				e.stopPropagation();
				console.log("clicked reply all");
				if ($("[data-test-id='add-update-editor']").length == 0) {
					click(".floating-editor [data-test-id='reply-link']");
				}
				async function processEmails(ccs, bccs) {
					
					// process cc and bcc
					var containers =[{
						container : ".floating-editor_cc-container",
						trigger: '[data-test-id="cc-trigger"]',
						emails : ccs
					},{
						container : ".floating-editor_bcc-container",
						trigger: '[data-test-id="bcc-trigger"]',
						emails: bccs
					},]
					var addedAny=false;
					for(var cidx=0;cidx<containers.length;cidx+=1){
						if (containers[cidx].emails.length > 0) {
							var ccContainer = $(containers[cidx].container+" a");
							var ccTrigger = $(containers[cidx].trigger);
							if (ccTrigger.length != 0) {
								click(ccTrigger);
							} else {
								click(ccContainer);
							}								
							//click($(containers[cidx].container));
							$(containers[cidx].container).addClass("inject-waiting");
							await sleep(600);
							var j=0;

							for(var dp =0;dp<2;dp+=1){
								var existingEmails = $(containers[cidx].container+" .ember-power-select-multiple-option").map(function(){return $(this).text().replace("×","").trim().toLowerCase();});
								console.log("existing",existingEmails);
								for(var i =0;i<containers[cidx].emails.length;i++){
									var c = containers[cidx].emails[i].trim().toLowerCase();
									if (dp>0){
										if ($.inArray(c,existingEmails)==-1){
											await sleep(1000);
										}
									}

									if ($.inArray(c,existingEmails)==-1){	
										console.info("adding "+c);
										sendLine(c);
										addedAny=true;										
										await sleep(300+dp*500);
										/*while($(".ember-power-select-option--loading-message").length>0){
											await sleep(300);
										}*/
									}
								}
				...
				var additional = $(this).closest("article").find(".update-box_footer_recipients_remaining a");
				if (additional.length != 0) {
					...
					var article = $(this).closest("article");
					setTimeout(function () {
						$(article).find(".update-box_other-recipients").children().map(function () {
							var txt = $(this).text().trim();
							if (txt == "CC Recipients")
								type = "(cc)";
							else if (txt == "BCC Recipients")
								type = "(bcc)";
							else {
								if (type == "(cc)")
									cc.push(txt);
						...
						processEmails(cc, bcc);
					}, 500);

				} else {
					var found = $(this).closest("article").find(".update-box_footer_recipients .update-box_footer_recipients_value").text().trim();
					found.split(",").map(function (i) {
						var email = beforeLast(i, "(");
						var type = afterLast(i, " ");
						if (type == "(cc)")
							cc.push(email);
						if (type == "(bcc)")
							bcc.push(email);
	...
	function displayTicketInfo(email, prependTo) {
		...
		detailsBox.html("loading...");
		$(prependTo).prepend(detailsBox);

		function appendRow(table, value) {
			var tr = $("<tr>");
		...
		function processResult(o) {
			if (o.deleted == false) {
				var table = $("<table>").addClass("inject inject-float-table");
				if (o.user.ImageUrl != "/i/userplaceholder") {
					appendRow(table, $("<img>").attr("src", o.user.ImageUrl));
				}
				appendRow(table, "<span class='inject-company'>"+o.org.Name+"</span>");
				appendRow(table, "<span class='inject-company-status'>"+o.org.Status+"</span>");
				appendRow(table, "<span class='inject-user-position'>"+o.position+"&nbsp;"+"</span>");
				//appendRow(table,""+ o.deleted);
				appendRow(table, $("<a target='_blank' href='" + TT_URL + "/organization/which/" + o.org.Id + "' >View</a>"));
				appendRow(table, $("<a target='_blank' href='" + TT_URL + "/account/setasuser?id=" + email + "' >Set as</a>"));
				appendRow(table, $("<a target='_blank' href='" + TT_URL + "/account/setrole/" + o.user.Id + "' >Login</a>"));
		...
		loadData(
			TT_URL + "/clientsuccess/HFUserInfo?search=" + email,
			function (data) {
			var os = data.Object;
			detailsBox.html("");
			var f = os.filter(function (i) {
					return i.deleted == false;
				});
			//if (f.length == 1) {
				updateHfData(f[0]);
			//}
			var anyImplementer=false;
			f.map(function (i) {
				processResult(i);
				/*
				if (i.org.Status=="Implementer")
			...
			appendErr("user details err");
		});
	}

	var showUserPassModal = function () {
		var tturl = TT_URL; //"https://traction.tools";//TT_URL
		//var url = tturl+"/account/login?browserExt=true&returnUrl=%2FError%2FIndex%3Fmessage%3DPlease%20close%20this%20window%20and%20refresh";
		var viewWindow = $(".inject-view-window");
		if (viewWindow.length == 0) {
			viewWindow = $("<div>").addClass("inject-view-window");
			$("body").append(viewWindow);
		}
		$(".inject-view-window").html("");
		var form = $("<form autocomplete='off'>");

		form.append($("<input autocomplete='off' placeholder='username' type='text' name='u' data-lpignore='true'>"));
		var p = $("<input autocomplete='off' type='text' placeholder='password' name='p' data-lpignore='true'>");
		form.append(p);
		var submit = $("<input autocomplete='off' type='button' value='TT login'>");
		var close = $("<input autocomplete='off' style='float:right;' type='button' value='close'>");
		form.append(submit);
		form.append(close);
		$(".inject-view-window").append(form);
		//try to disable autofill...
		setTimeout(function () {$(p).attr("type", "password");}, 1000);
		...
		submit.on("click", function (e) {
			...
			var settings = {
				...
				"data" : {
					...
					"password" : $(form).find("[name='p']").val()
				},
				success : function (response) {
					localStorage.setItem('tt-token', response.access_token);
					$(".inject-view-window").remove();

				},
				error : function (a, b, c) {
					var response = a.responseJSON;
					form.append($("<div>Err:" + response.error_description + "</div>"));
				}
			}
			$.ajax(settings);
		});
	...
	function loadData(url,postData, success, error) {
		...
		var headers = {};
		if (url.indexOf(TT_URL) != -1 && localStorage.getItem('tt-token') !== null) {
	...
	async function updateHfDataField(fieldName, value) {
		var existing = $(".custom-field_label-container:contains('" + fieldName + "')").closest(".custom-field").find(".custom-field_value").text();

		if (existing.trim() == "-" && typeof(value)!=="undefined" && (""+value).trim()!="") {
			...
			$(".custom-field_label-container:contains('" + fieldName + "')").closest(".custom-field").find(".custom-field_label-container").click();
			var input = $(".custom-field_label-container:contains('" + fieldName + "')").closest(".custom-field").find("input");
			input.val(value);
			/*click(input);
			sendText(value);
			await sleep(400);
	...
	function waitFor(selector, callback, count) {
		...
		if (typeof(count) === "undefined") {
			count = 0;
			console.info("looking...");
		}
		if (count >= 50) {
			console.error("WaitFor failed after 50 tries");
			return;
		}
		if (s.length == 0) {
			setTimeout(function () {
				waitFor(selector, callback, count + 1);
			}, 500);
		} else {
			console.info("found.");
			callback(s);
	...
	function afterLast(str, lookup) {
		return str.substr(str.lastIndexOf(lookup) + 1);
	}
	function beforeLast(str, lookup) {
		return str.substr(0, str.lastIndexOf(lookup) - 1);
	}
	function onMatch(urlSubstr, onmatch) {
		function testMatch(url, sub, onmatch) {
			if (url.indexOf(sub) != -1) {
				console.info("match:" + sub);
				onmatch(url);
				return true;
			}
			return false;
		}
		var tablink = config.url;
		if (Array.isArray(urlSubstr)) {
			for (var i in urlSubstr) {
				if (urlSubstr.hasOwnProperty(i)) {
					if (testMatch(tablink, urlSubstr[i], onmatch))
						return;
				}
			}
		} else {
			testMatch(tablink, urlSubstr, onmatch);
		}
	}
	...
	function sendTextRaw(text){
		chrome.runtime.sendMessage({
			...
			if (response != "ok") {
				console.error("sendText error:", response, text);
			}
		});
	}
