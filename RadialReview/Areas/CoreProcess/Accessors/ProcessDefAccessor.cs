...
namespace RadialReview.Areas.CoreProcess.Accessors {
	public class ProcessDefAccessor : IProcessDefAccessor {
		...
		public async Task<ProcessDef_Camunda> ProcessStart(UserOrganizationModel caller, long processId) {
			try {
				using (var s = HibernateSession.GetCurrentSession()) {
					using (var tx = s.BeginTransaction()) {
		...
		public async Task<bool> ProcessSuspend(UserOrganizationModel caller, string processInsId, bool isSuspend) {
			bool result = false;
			try {
				using (var s = HibernateSession.GetCurrentSession()) {
					using (var tx = s.BeginTransaction()) {
						...
						if (getProcessInsDetail != null) {
							// call Comm Layer
							CommClass commClass = new CommClass();
							var startProcess = await commClass.ProcessSuspend(processInsId, isSuspend);
		...
		public async Task<long> CreateProcessDef(ISession s, PermissionsUtility perms, string processName) {
			try {
				...
				// create permItem
				PermissionsAccessor.CreatePermItems(s, perms.GetCaller(), PermItem.ResourceType.CoreProcess, processDef.Id, PermTiny.Creator(), PermTiny.Admins());
		...
		public async Task<TaskViewModel> CreateTask(UserOrganizationModel caller, long localId, TaskViewModel model) {
			using (var s = HibernateSession.GetCurrentSession()) {
				...
				var created = await CreateTask(s, perm, localId, model);
				return created;
			}
		}

		public async Task<TaskViewModel> CreateTask(ISession s, PermissionsUtility perm, long localId, TaskViewModel model) {
