		...
		public static AngularRecurrence GetAngularRecurrence(UserOrganizationModel caller, long recurrenceId, bool includeScores = true, bool includeHistorical = true, bool fullScorecard = true, DateRange range = null, bool forceIncludeTodoCompletion = false) {
			using (var s = HibernateSession.GetCurrentSession()) {
				using (var tx = s.BeginTransaction()) {
					...
					var scoresAndMeasurables = GetScorecardDataForRecurrence(s, perms, recurrenceId, true, range: scorecardRange, getMeasurables: true, getScores: includeScores, forceIncludeTodoCompletion: forceIncludeTodoCompletion);
				...
				using (var tx = s.BeginTransaction()) {
					using (var rt = RealTimeUtility.Create()) {
						...
						if (model.Type == typeof(AngularIssue).Name) {
							...
							await CompleteTodo(s, perms, rt, model.Id);
