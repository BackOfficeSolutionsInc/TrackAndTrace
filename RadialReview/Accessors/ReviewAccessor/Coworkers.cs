			...
			public class FromAccountabilityChart {
					...
					if (reviewee.Type == OriginType.Organization) {
						...
						if (!revieweesNodes.Any()) {
							...
							foreach (var revieweeNode in revieweesNodes) {
								var directReports = revieweeNode.GetDirectChildren().Select(dr => new AccRelationship() {
								...
								var peers = AngularTreeUtil.GetDirectPeers(tree.Root, revieweeNode.Id).Select(peer => new AccRelationship() {
				...
				public static RelationshipCollection GetRelationshipsForNode(AngularAccountabilityChart tree, Reviewer reviewer) {
					...
					foreach (var reviewerNode in reviewersNodes) {
						var directReports = reviewerNode.GetDirectChildren().Select(dr => new AccRelationship() {
				...
				public static List<Reviewee> FilterTreeByTeam(AngularAccountabilityChart tree, OrganizationTeamModel team,List<UserOrganizationModel> members) {
					...
					if (team.Type == TeamType.Managers)
						return AngularTreeUtil.GetAllNodes(tree.Root).Where(x => x.GetDirectChildren().Any()).Select(x => new Reviewee(x)).ToList();
					...
					if (team.Type == TeamType.Subordinates)
						return AngularTreeUtil.FindUsersNodes(tree.Root, team.ManagedBy).SelectMany(x => x.GetDirectChildren().Union(x.AsList())).Select(x => new Reviewee(x)).ToList();
