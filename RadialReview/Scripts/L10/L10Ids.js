...
function sortIssueBy(recurId, issueList, sortBy, title, mult) {
	...
	var dict = {};
	$(issueList).children().each(function (i) {
		dict[+$(this).attr("data-issue")] = i;
	});
	...
	$(issueList).children().detach().sort(function (a, b) {
		if (sortBy == "data-priority") {
			...
			if (bb == 0)
				bb = Number.MAX_VALUE;
			if (aa === bb) {
				return dict[+$(a).attr("data-issue")] - dict[+$(b).attr("data-issue")];
				//var ma  =$(a).attr("data-message").toUpperCase();
				//var mb  =$(b).attr("data-message").toUpperCase();
				//if (ma==mb){
				//	return mult*(+$(a).attr("data-issue")-(+$(b).attr("data-issue")));
				//}
	...
	refreshRankTimer = setTimeout(function () {
		...
		for (var i = 0; i < ranks.length; i++) {
			...
			if (cur > expected) {
				...
				row.data("rank", expected);
				row.attr("data-rank", expected);
...
function floatTopThreeIssues(id) {
	try {
		var orderedMatchCount = 0;
		var selected = $(".ids .issue-row").filter(function (i) {
			return !$(this).is("[data-checked=true]") && !$(this).is("[data-checked=True]");
		}).filter(function (i) {
			var rank = +$(this).attr("data-rank");

			if (i + 1 == rank) {
				if (i > orderedMatchCount)
					return true;
				orderedMatchCount += 1;
				return false;
			}
		...
		selected.animate({ left: '+=100px', opacity: 0 }, 200).slideUp(250);
		setTimeout(function () {
			sortIssueByCurrent(recurrenceId, $('.issues-list'));
			setTimeout(function () {
				selected.slideDown(250).animate({ left: '-=100px', opacity: 1 }, 200);
				$('.slider-container.level-10').animate({ scrollTop: 0 }, 500);
			}, 150);
		}, 450);
	} catch (e) {
		console.warn(e);
...
$(function () {
	...
	$("body").on("mousedown", ".issue-row .rank123", function (e) {
		...
		rankTimer[id] = setTimeout(function () {
			...
			var d = { rank: pp, time: new Date().getTime() };
			//console.log("D - " + pp);
			console.info("sending rank:" + pp);

			if (pp == orig) {
				console.warn("Rank update was the same!");
				return;
			}
