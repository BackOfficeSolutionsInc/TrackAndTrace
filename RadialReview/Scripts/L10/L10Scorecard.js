...
function makeXEditable_Scorecard(selector) {
	...
	$(selector).editable({
		...
		success: function (data, newVal) {
			...
			if (isUnit) {
				$("[data-measurable=" + $(this).data("measurable") + "] .unit").html((currencySymbol[newVal] || "").toLowerCase());
			}
...
function addScChart(id) {
	...
	var name = row.find(".measurable").first().clone().children().filter(function () {
		return !$(this).is(".edit-formula");
	}).text();
	
	if (!(id in addScChart.colorLookup)) {
		addScChart.colorLookup[id] = addScChart.allColors[(addScChart.curChart++) % addScChart.allColors.length];
	}

	addScChart.chart[id] = {
		data: datas,
		legend: name,
		color: addScChart.colorLookup[id]
	};
	reapplyScChart();
}

addScChart.curChart = 0;
addScChart.chart = {};
addScChart.colorLookup = {};
addScChart.allColors = ["#4040e8", "#05b378", "#db4437", "#f8b128", "#5c5c5c", "steelblue", "#f673bf", "#0b73b0", "#006400", "#92514f"];
...
function reapplyScChart() {
	...
	Object.keys(addScChart.chart).map(function (x) {
		...
		colors.push(addScChart.chart[x].color);
	});
	//debugger;
	...
	if (typeof (addScChart.mainChart) === "undefined") {
		addScChart.scUpdate = $("body").on("change.reapplyScChart", ".score input", function () {
			...
			setTimeout(function () {
				var row = $("tr[data-measurable='" + mid + "'] .chart .chart-btn");
				if (row.is(".selected")) {
					addScChart(mid);
				}
			}, 200);
