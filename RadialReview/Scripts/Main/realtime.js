...
$(function () {
	waitUntil(function () {
		return typeof ($.connection) !== "undefined" && typeof ($.connection.meetingHub) !== "undefined";
	}, function () {
		meetingHub = $.connection.meetingHub;
		$.connection.hub.start(Constants.StartHubSettings).done(function () {
			meetingHub.server.joinMessageHub($.connection.hub.id).done(function () {
				console.log("connected to meetingHub 1")
			});
		});
		...
		meetingHub.client.showAlert = function (mObj) {
			showAlert(mObj);
		};
		//Server can notify users of upcoming meetings
		meetingHub.client.NotifyOfMeetingStart = function (recurrenceId, recurrenceName) {
