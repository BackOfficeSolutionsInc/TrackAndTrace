...
(function (Charts) {
    ...
    var Pie = (function (_super) {
        ...
        Pie.prototype.Initialize = function (data, yTitle) {
            ...
            var _this = this;
            if (typeof yTitle === "undefined") { yTitle = ""; }
            var values = [];
            ...
            }).attr("dy", ".35em").style({ "text-anchor": "middle", "font-size": "1.75em", "fill": this.TextColor }).text(function (d) {
                return d.data.group;
            });
            ...
            g.append("text").attr("transform", function (d) {
                ...
                shift[1] += 10;
                return "translate(" + shift + ")";
            }).attr("dy", ".55em").style({ "text-anchor": "middle", "font-size": "1em", "fill": this.TextColor }).text(function (d) {
                return d.data.y;
            });
    ...
    var Line = (function (_super) {
        ...
        Line.prototype.Initialize = function (data, yTitle) {
            ...
            svg.selectAll(".dot").data(data).enter().append("circle").attr("class", function (d) {
                return "dot " + d.classed;
            }).attr("cx", line.x()).attr("cy", line.y()).attr("r", 3.5);
    ...
    var Histogram = (function (_super) {
        ...
        Histogram.prototype._Initialize = function (svg, data, x, y, xAxis, classed) {
            ...
            var bar = svg.selectAll(".bar ." + classed).data(data).enter().append("g").attr("class", function (s) {
                return "bar " + classed;
        ...
        Histogram.prototype.InitializeWidth = function (values2, binWidth) {
            var _this = this;
            if (typeof binWidth === "undefined") { binWidth = 10; }
            ...
            for (var j in groups) {
                ...
                maxFreq = Math.max(maxFreq, d3.max(data, function (d) {
                    return d.y;
                }));
            }
            var first = true;
            for (var j in groups) {
                ...
                var xAxis = d3.svg.axis().scale(x).orient("bottom").ticks(0).tickFormat(function (d) {
                    return _this.LabelFunction(d);
                });
                if (first)
                    xAxis = d3.svg.axis().scale(x).orient("bottom").tickValues(useBins).tickFormat(function (d) {
                        return _this.LabelFunction(d);
                    });
                first = false;
