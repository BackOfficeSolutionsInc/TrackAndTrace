...
(function (factory) {
	...
	DraggableGridList.prototype = {
		...
		_onStop: function (event, ui) {
			...
			this._applyPositionToItems(true);
		...
		_calculateCellSize: function () {
			if (this.options.direction === "horizontal") {
				this._cellHeight = Math.max(Math.floor(this.$element.height() / this.options.lanes), this.options.minHeight);
				this._cellWidth = Math.max(this._cellHeight * this.options.widthHeightRatio, this.options.minWidth);
			} else {
				this._cellWidth = Math.max(Math.floor(this.$element.width() / this.options.lanes), this.options.minWidth);
				this._cellHeight = Math.max(this._cellWidth / this.options.widthHeightRatio, this.options.minHeight);
			}
		...
		_applyPositionToItems: function (noDrag) {
			var mult = .51;
			if (noDrag) {
				mult =.51;
			}
			...
			if (this.options.direction === "horizontal") {
				this.$element.width(
				  (this.gridList.grid.length + this._widestItem * mult) * this._cellWidth);
			} else {
				this.$element.height(
				  (this.gridList.grid.length + this._tallestItem * mult) * this._cellHeight);
			}
		},
