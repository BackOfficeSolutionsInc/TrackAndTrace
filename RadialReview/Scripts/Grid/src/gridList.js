(function (root, factory) {
	...
	GridList.prototype = {
		...
		_updateItemSize: function (item, width, height) {
			...
			$(item.$element).data("w", item.w);
			$(item.$element).data("h", item.h);
		...
		_setItemPosition: function (item, position) {
			...
			$(item.$element).attr("data-x", item.x);
			$(item.$element).attr("data-y", item.y);
			$(item.$element).data("x", item.x);
			$(item.$element).data("y", item.y);
		}
	};
